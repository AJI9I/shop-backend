...
/**
 * Кастомный контроллер для обработки ошибок, включая 404
 */
@Controller
...
public class CustomErrorController implements ErrorController {
    ...
    public String handleError(HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        // Если ERROR_REQUEST_URI не установлен, используем текущий URI (для forward из GlobalExceptionHandler)
        if (url == null) {
        ...
        // Если статус не установлен, но это не /error, считаем это 404
        if (status == null && !"/error".equals(url)) {
            log.warn("404 Not Found (no status code): {}", fullUrl);
        ...
        if (status != null) {
            ...
            if (statusCode == HttpStatus.NOT_FOUND.value()) {
                // Логируем 404 ошибку
                log.warn("404 Not Found: {}", fullUrl);
                logNotFoundError(request, fullUrl);
        ...
        model.addAttribute("pageDescription", "Произошла ошибка");
